...
Critical Message Logging - MTE State change
    ...
    wait GMI message after time    CRITICAL.*Normal Processing.*MTE.*ReportSituation    ${currDateTime}    waittime=2    timeout=100
    wait GMI message after time    CRITICAL.*Watchdog event.*MTE.*ReportSituation    ${currDateTime}    waittime=2    timeout=100
    [Teardown]    MTE Failover Case Teardown    ${master_ip}
