#---------------------------------------------
# common variables for the venue
#----------------------------------------------

# Remote Machine Settings
CHE_A_IP       = '167.76.120.40'
CHE_B_IP       = '167.76.120.40'
CHE_IP         = CHE_A_IP
CHE_PORT       = '22'
FMSCMD_PORT    = '25000'
SCWCLI_PORT    = '27000'
USERNAME       = 'root'
PASSWORD       = 'Pegestech01'
MTE            = 'VENUEnnM'
FH             = 'VENUEnnF'
VENUE_DIR      = '/ThomsonReuters/Venues/'
BASE_DIR       = '/ThomsonReuters/'
REMOTE_TMP_DIR = '/tmp'
TOOLS_DIR      = '/usr'

# Playback Machine Settings
PLAYBACK_MACHINE_IP = '167.76.115.28'
PLAYBACK_PORT     = '22'
PLAYBACK_BIND_IP_A = '192.168.63.16'
PLAYBACK_BIND_IP_B = '192.168.64.16'
PLAYBACK_PPS      = '1000'
PLAYBACK_USERNAME = 'root'
PLAYBACK_PASSWORD = 'Pegestech1'
PLAYBACK_PCAP_DIR ='/data/'

# Local Machine Settings
LOCAL_DAS_DIR    = 'C:\\Program Files\\Reuters Test Tools\\DAS'
LOCAL_PMAT_DIR   = 'C:\\PMAT\\x64'
LOCAL_FMS_DIR    = 'D:\\tools\\FMSCMD\\config\\DataFiles\\Groups'
LOCAL_FMS_BIN    = 'D:\\tools\\FMSCMD\\bin'
LOCAL_SCWCLI_BIN = 'D:\\tools\\SCWatchdog_v1.7.0'
LOCAL_TMP_DIR    = 'D:\\temp'

# My vagrant VM settings
#CHE_A_IP         =  '10.91.161.246'
#CHE_B_IP         =  '10.91.161.246'
CHE_A_IP         =  '167.76.115.78'
CHE_B_IP         =  '167.76.115.79'
#CHE_A_IP         =  '10.40.248.228'
#CHE_B_IP         =  '10.40.248.228'
CHE_IP           = CHE_A_IP
#USERNAME         =  'root'
#PASSWORD         =  'vagrant'
MTE              = 'TDDS01M'
FH               = 'TDDS01F'
#MTE              = 'HKF02M'
#FH               = 'HKF01F'
#MTE              = 'MFDS1M'
#FH               = 'MFDS1F'
PLAYBACK_MACHINE_IP = '167.76.115.239'
PLAYBACK_BIND_IP_A = '192.168.63.85'
PLAYBACK_BIND_IP_B = '192.168.64.85'
LOCAL_TMP_DIR    = 'C:\\temp'
LOCAL_FMS_DIR    = 'C:\\tools\\FMSCMD\\config\\DataFiles\\Groups'
LOCAL_FMS_BIN    = 'C:\\tools\\FMSCMD\\bin'
LOCAL_SCWCLI_BIN = 'C:\\tools\\SCWatchdog_v1.7.0\\bin\\x64'
