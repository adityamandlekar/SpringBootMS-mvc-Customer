...
#         print 'DEBUG cmd=%s, rc=%s, stdout=%s stderr=%s' %(cmd,rc,stdout,stderr)
    ...
    def wait_and_get_OTFC_ric_from_smf(self,message,timeRef, waittime=2, timeout=60):
        ...
        Return : OTFC RIC name

        Examples:
        | ${dt}= | get date and time |
        | ${Ric} |   wait and get OTFC RIC from smf   | ${MTE}.+Created on-the-fly Real-time  | ${dt} |
        ...
        while time.time() <= maxtime:            
            ...
            if (len(retMessages) > 0):
                logContents = retMessages[-1].split(';')
                if (len(logContents) >= 4):
                    if logContents[0].strip() >= refDate and logContents[1].strip() >= refTime:
                        otfContents = logContents[3].split(' ') 
                        if (len(otfContents) >= 7):
                            return otfContents[6].strip()
            time.sleep(waittime)
        raise AssertionError('*ERROR* Fail to get pattern \'%s\' from smf log before timeout %ds' %(message, timeout)) 
