...
#                     print 'DEBUG fileStartSize=%s fileCurrSize=%s' %(fileStartSize,fileCurrSize)
    ...
    def get_pcap_frames(self, pcapfilelist):
        """total frames from remote pcap file list.
        Arguement pcapfilelist list of pcap file.
        return sum of frames for input pcap files
        """
        frameSum = 0 
        for fileitem in pcapfilelist:
            cmd = "tcpdump -r  %s | wc -l" %fileitem
            retInfo = _exec_command(cmd)
            stdout = retInfo[0]
            frameSum += int(stdout)  
        
        if frameSum < 1:
            raise AssertionError('*ERROR* Failed to get frame info from file list %s' % (','.join(pcapfilelist)))
               
        return frameSum
    
    def compare_pcap_frames_with_configured_size(self, pcapfilelist, maxStreamBuffer):
        """ Check each pcap file frame number in pcapfilelist should equal or less than maxStreamBuffer
        Arguement maxStreamBuffer defined in grs configuration file.
                  pcapfilelist file list generated by GRS.
        """
        for fileitem in pcapfilelist:
            cmd = "tcpdump -r  %s | wc -l" %fileitem
            retInfo = _exec_command(cmd)
